"use client";

import { useEffect, useState, ReactNode } from "react";
import { CartProvider } from "./CartProvider";
import { FavoritesProvider, useFavorites } from "./FavoritesProvider";
import { Header } from "./Header";
import { CartDrawer } from "./CartDrawer";
import { FavoritesDrawer } from "./FavoritesDrawer";

interface DrawerProduct {
  id: string;
  name: string;
  description: string;
  price: number;
  images: string[];
  inStock: boolean;
}

function Favorites({ products }: { products?: DrawerProduct[] }) {
  const { favDrawerOpen } = useFavorites();
  const [fetched, setFetched] = useState<DrawerProduct[] | null>(null);

  useEffect(() => {
    if (products || !favDrawerOpen || fetched) return;
    fetch("/api/products", { cache: "no-store" })
      .then((r) => r.json())
      .then(setFetched)
      .catch(() => {});
  }, [products, favDrawerOpen, fetched]);

  return <FavoritesDrawer products={products ?? fetched ?? []} />;
}

export function SiteShell({ children, products }: { children: ReactNode; products?: DrawerProduct[] }) {
  return (
    <FavoritesProvider>
      <CartProvider>
        <Header />
        <main className="flex-1">{children}</main>

        <footer className="border-t border-neutral-100 bg-neutral-50">
          <div className="px-4 py-8 text-center">
            <p className="text-xs text-neutral-400">
              AUTOSHINE.TJ — Детейлинг Маркет
            </p>
            <p className="text-xs text-neutral-300 mt-1">
              ш. Душанбе бозори Кушониён блоки 14 моғозаи 4492
            </p>
            <a href="/admin" className="inline-flex items-center gap-1.5 mt-4 px-4 py-2 rounded-lg bg-neutral-200 text-neutral-500 text-xs font-medium hover:bg-neutral-300 transition-colors">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
                <path d="M7 11V7a5 5 0 0110 0v4" />
              </svg>
              Админ-панель
            </a>
          </div>
        </footer>

        <CartDrawer />
        <Favorites products={products} />
      </CartProvider>
    </FavoritesProvider>
  );
}
